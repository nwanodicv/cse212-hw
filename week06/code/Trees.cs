public static class Trees
{
    ...
    private static void InsertMiddle(int[] sortedNumbers, int first, int last, BinarySearchTree bst)
    {
        ...
        if (first > last)
        {
            return; // Base case: if the range is invalid, return
        }
        int mid = (first + last) / 2; // Find the middle index
        bst.Insert(sortedNumbers[mid]); // Insert the middle value into the BST
        ...
        // This ensures that the BST remains balanced by inserting the middle elements first
    }
}